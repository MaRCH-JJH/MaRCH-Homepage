...
### 소식(뉴스) 본문에 사진 여러 장 넣기

"최신소식" 글 본문(마크다운 에디터)에서 사진 버튼으로 사진을 연달아 여러 장 넣으면,
**자동으로 한 줄에 2장씩 나란히 배치되고 크기도 원래보다 작게 조절됩니다** (기존에는 사진 한 장이
항상 화면 전체 폭을 차지했습니다). 특별한 문법을 배울 필요 없이 지금처럼 사진 버튼을 그냥
연달아 누르기만 하면 됩니다. 사진을 3장 연달아 넣으면 2장+1장으로 자동으로 줄바꿈됩니다.

특정 사진 한 장만 화면 전체 폭으로 크게 보이게 하고 싶은 경우처럼 자동 배치와 다르게 하고
싶은 경우는 개발자에게 요청해 주세요.
